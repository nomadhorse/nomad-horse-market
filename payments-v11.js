...
(() => {
  ...
  function openReceiptReadyModal(dealId, paymentId) {
    ...
    backdrop.innerHTML = `
      <div class="nh-modal" role="dialog" aria-modal="true" aria-label="Pagamento salvo">
        ...
        <div class="nh-ok">Pagamento salvo com sucesso. O recibo pode ser baixado em PDF agora ou novamente pelo histórico.</div>
      ...
      ? payments.map(p => `
          <div class="nh-history-row" style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,.08);align-items:flex-start">
            <span style="display:flex;flex-direction:column;gap:4px;min-width:0">
              ...
              <button type="button" class="nh-mini-btn secondary" data-receipt-pdf="${esc(p.id)}" style="align-self:flex-start;margin-top:5px;padding:7px 10px">RECIBO PDF</button>
            </span>
            <strong>${money(p.amount)}</strong>
          </div>`).join('')
  ...
  async function init() {
    try {
      ...
      window.nomadHorseFinance = { reload: loadFinance, client: db, editSchedule: openScheduleModal, newProject: openProjectSetupModal, registerPayment: openPaymentModal, paymentHistory: openPaymentHistoryModal, receiptPdf: generateReceiptPdf };
      await loadFinance();
