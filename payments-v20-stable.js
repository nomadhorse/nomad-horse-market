/* Nomad Horse Market — módulo financeiro de projetos (v20-stable)
...
(() => {
  ...
  // Backup/restauração v20 — implementação estável, sem observadores globais.
  function nhBackupCounts(backup) {
    ...
    return names.reduce((acc, name) => { acc[name] = Array.isArray(tables[name]) ? tables[name].length : 0; return acc; }, {});
  }

  function nhValidateBackup(backup) {
  ...
  function nhBackupStamp(date = new Date()) {
  ...
  function nhToast(message, kind='ok') {
    document.getElementById('nhBackupToast')?.remove();
    const toast=document.createElement('div');
    toast.id='nhBackupToast';
    toast.setAttribute('role','status');
    toast.style.cssText=`position:fixed;left:18px;right:18px;bottom:22px;z-index:99999;padding:14px 16px;border-radius:14px;font-weight:800;text-align:center;background:${kind==='error'?'#4a2020':'#2b261d'};color:#fff;border:1px solid ${kind==='error'?'rgba(255,120,120,.35)':'rgba(212,164,75,.45)'};box-shadow:0 12px 30px rgba(0,0,0,.35);pointer-events:none`;
    toast.textContent=message;
    document.body.appendChild(toast);
    setTimeout(()=>toast.remove(),3200);
  }

  function nhDownloadJson(filename, value) {
  ...
  async function nhExportSecureBackup(event) {
    event?.preventDefault?.();
    event?.stopPropagation?.();
    if (!db) return nhToast('Aguarde o painel terminar de carregar.','error');
    try {
      const {data:sessionData}=await db.auth.getSession();
      if (!sessionData?.session) return nhToast('Entre no painel como administrador para exportar.','error');
      const {data,error}=await db.rpc('export_nomad_backup');
      if (error) throw error;
      nhValidateBackup(data);
      nhDownloadJson(`nomad-horse-backup-${nhBackupStamp()}.json`,data);
      const now=new Date().toISOString();
      localStorage.setItem('nhSecureLastBackupAt',now);
      localStorage.setItem('nhm_last_backup_at',now);
      const total=Object.values(nhBackupCounts(data)).reduce((s,n)=>s+Number(n||0),0);
      nhToast(`Backup exportado • ${total} registro(s).`);
    } catch(err) {
      console.error('[Nomad Horse Backup] exportação',err);
      nhToast(`Falha ao exportar: ${err?.message||err}`,'error');
    }
  }

  function nhReadTextFile(file) {
  ...
  function nhCloseRestoreModal(){ document.getElementById('nhRestoreBackupModal')?.remove(); }

  function nhOpenRestorePreview(backup,fileName='backup.json') {
    nhValidateBackup(backup);
    nhCloseRestoreModal();
    const counts=nhBackupCounts(backup);
    ...
    backdrop.innerHTML=`
      <div class="nh-modal" role="dialog" aria-modal="true" aria-label="Restaurar backup">
        <div class="nh-modal-head"><div><h3>Restaurar backup</h3><div class="muted">${esc(fileName)}</div></div><button type="button" class="nh-close" id="nhCloseRestoreBackup">×</button></div>
        <div class="nh-project-note"><strong>Restauração segura por mesclagem</strong><br>O arquivo será validado antes de qualquer gravação. Registros mais novos que não estejam no backup não serão apagados.</div>
        <div class="nh-schedule-total" style="margin-top:12px"><div><small>Registros no arquivo</small><strong>${esc(total)}</strong></div><div><small>Projetos</small><strong>${esc(counts.deals)}</strong></div></div>
    ...
    document.body.appendChild(backdrop);
    document.getElementById('nhCloseRestoreBackup')?.addEventListener('click',nhCloseRestoreModal);
    backdrop.addEventListener('click',e=>{if(e.target===backdrop) nhCloseRestoreModal();});
    document.getElementById('nhConfirmRestoreBackup')?.addEventListener('click',async(e)=>{
      const btn=e.currentTarget, box=document.getElementById('nhRestoreBackupError');
      try {
        btn.disabled=true; btn.textContent='RESTAURANDO…'; if(box) box.innerHTML='';
        const {error}=await db.rpc('restore_nomad_backup',{p_backup:backup});
        if(error) throw error;
        btn.textContent='RESTAURAÇÃO CONCLUÍDA';
        localStorage.setItem('nhSecureLastRestoreAt',new Date().toISOString());
        await loadFinance();
        setTimeout(()=>{ nhCloseRestoreModal(); nhToast('Backup restaurado com sucesso.'); },300);
      } catch(err) {
        console.error('[Nomad Horse Backup] restauração',err);
        if(box) box.innerHTML=`<div class="nh-error">Não foi possível restaurar: ${esc(err?.message||err)}</div>`;
        btn.disabled=false; btn.textContent='RESTAURAR / MESCLAR DADOS';
      }
    });
  }

  function nhEnsureRestoreInput() {
    let input=document.getElementById('nhRestoreBackupInput');
    if(input) return input;
    input=document.createElement('input');
    input.id='nhRestoreBackupInput'; input.type='file'; input.accept='.json,application/json'; input.style.display='none';
    input.addEventListener('change',async()=>{
      const file=input.files?.[0]; input.value=''; if(!file) return;
      try { const text=await nhReadTextFile(file); nhOpenRestorePreview(JSON.parse(text),file.name); }
      catch(err){ console.error('[Nomad Horse Backup] leitura',err); nhToast(`Backup inválido: ${err?.message||err}`,'error'); }
    });
    document.body.appendChild(input); return input;
  }

  function nhOpenRestorePicker(event) {
    event?.preventDefault?.(); event?.stopPropagation?.();
    if(!db) return nhToast('Aguarde o painel terminar de carregar.','error');
    nhEnsureRestoreInput().click();
  }

  function nhBindBackupControls() {
    const admin=getAdminSection(); if(!admin) return;
    const exportBtn=[...admin.querySelectorAll('button')].find(btn=>/exportar backup/i.test(btn.textContent||''));
    if(exportBtn && exportBtn.dataset.nhBackupV20!=='1') {
      exportBtn.dataset.nhBackupV20='1';
      exportBtn.removeAttribute('onclick'); exportBtn.onclick=null;
      exportBtn.addEventListener('click',nhExportSecureBackup);
    }
    if(exportBtn && !document.getElementById('nhRestoreBackupBtn')) {
      const restore=document.createElement('button');
      restore.id='nhRestoreBackupBtn'; restore.type='button'; restore.className=exportBtn.className||'btn ghost'; restore.textContent='Restaurar backup';
      restore.addEventListener('click',nhOpenRestorePicker);
      exportBtn.insertAdjacentElement('afterend',restore);
    }
  }
  ...
  async function init() {
    try {
      ...
      window.nomadHorseFinance = { reload: loadFinance, client: db, editSchedule: openScheduleModal, newProject: openProjectSetupModal, registerPayment: openPaymentModal, paymentHistory: openPaymentHistoryModal, receiptPdf: generateReceiptPdf, shareReceipt, collectionDashboard: buildCollectionDashboard, exportBackup: nhExportSecureBackup, restoreBackup: nhOpenRestorePicker };
      nhBindBackupControls();
      await loadFinance();

      // Recarrega ao entrar no painel, inclusive em navegação SPA.
      document.addEventListener('click', (e) => {
        const el = e.target.closest?.('[data-view="admin"]');
        if (el) setTimeout(() => { loadFinance(); nhBindBackupControls(); }, 120);
      });
