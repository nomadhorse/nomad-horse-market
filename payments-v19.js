/* Nomad Horse Market — módulo financeiro de projetos (v18)
...
(() => {
  ...
  function backupTableCounts(backup) {
    const tables = backup?.tables && typeof backup.tables === 'object' ? backup.tables : {};
    const names = ['leads','listings','listing_images','deals','deal_payments','deal_installments','client_history','commercial_goals','deal_collection_log'];
    return names.reduce((acc, name) => {
      acc[name] = Array.isArray(tables[name]) ? tables[name].length : 0;
      return acc;
    }, {});
  }

  function validateBackupFile(backup) {
    if (!backup || typeof backup !== 'object' || Array.isArray(backup)) throw new Error('Arquivo JSON inválido.');
    if (backup.app !== 'Nomad Horse Market') throw new Error('Este arquivo não é um backup do Nomad Horse Market.');
    if (Number(backup.format_version) !== 1) throw new Error('Versão de backup não suportada.');
    if (backup.project_ref !== 'ndbekzgxdfuhjlocipiv') throw new Error('Este backup pertence a outro projeto.');
    if (!backup.tables || typeof backup.tables !== 'object') throw new Error('Backup sem tabelas válidas.');
    return true;
  }

  function backupFileStamp(date = new Date()) {
    const d = localDateISO(date);
    const h = String(date.getHours()).padStart(2,'0');
    const m = String(date.getMinutes()).padStart(2,'0');
    return `${d}-${h}${m}`;
  }

  function backupToast(message, kind = 'ok') {
    document.getElementById('nhBackupToast')?.remove();
    const toast = document.createElement('div');
    toast.id = 'nhBackupToast';
    toast.setAttribute('role', 'status');
    toast.style.cssText = `position:fixed;left:18px;right:18px;bottom:22px;z-index:99999;padding:14px 16px;border-radius:14px;font-weight:800;text-align:center;background:${kind === 'error' ? '#4a2020' : '#2b261d'};color:#fff;border:1px solid ${kind === 'error' ? 'rgba(255,120,120,.35)' : 'rgba(212,164,75,.45)'};box-shadow:0 12px 30px rgba(0,0,0,.35)`;
  ...
  async function downloadJson(filename, value) {
    ...
    // Em Android/PWA, compartilhar o arquivo é mais estável que forçar um download
    // por blob e evita travar a interface depois que o seletor do sistema fecha.
    try {
      if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
        await navigator.share({
          title: 'Backup Nomad Horse Market',
          text: 'Backup seguro do Nomad Horse Market.',
          files: [file]
        });
        return 'shared';
      }
      ...
      console.warn('[Nomad Horse Backup] compartilhamento indisponível, usando download', err);
    }
  ...
  function normalizeUiAfterBackup() {
    // Garante que nenhum estado visual do Android/PWA deixe a interface bloqueada.
  ...
  function refreshBackupStatusLabel(isoValue = null) {
    const stamp = isoValue || localStorage.getItem('nhSecureLastBackupAt');
    if (!stamp) return;
    const admin = getAdminSection();
    if (!admin) return;
    const leafs = [...admin.querySelectorAll('*')].filter(el => !el.children.length && /último backup/i.test(el.textContent || ''));
    leafs.forEach(label => {
      const box = label.parentElement;
      const strong = box?.querySelector('strong');
      if (strong) strong.textContent = dateTimeBR(stamp);
    });
  }

  async function exportSecureBackup() {
    if (!db) throw new Error('Banco de dados ainda não iniciou.');
    const { data: sessionData } = await db.auth.getSession();
    if (!sessionData?.session) {
      alert('Entre no painel como administrador para exportar o backup.');
      return;
    }
    try {
      const { data, error } = await db.rpc('export_nomad_backup');
      if (error) throw error;
      validateBackupFile(data);
      const filename = `nomad-horse-backup-${backupFileStamp()}.json`;
      const result = await downloadJson(filename, data);
      if (result === 'cancelled') {
        normalizeUiAfterBackup();
        backupToast('Exportação cancelada. Nenhum dado foi alterado.');
        return;
      }
      const now = new Date().toISOString();
      localStorage.setItem('nhSecureLastBackupAt', now);
      localStorage.setItem('nhm_last_backup_at', now);
      refreshBackupStatusLabel(now);
      const counts = backupTableCounts(data);
      const total = Object.values(counts).reduce((s,n) => s + Number(n || 0), 0);
      normalizeUiAfterBackup();
      backupToast(`Backup preparado com sucesso • ${total} registro(s).`);
    } catch (err) {
      console.error('[Nomad Horse Backup] exportação', err);
      normalizeUiAfterBackup();
      backupToast(`Não foi possível exportar o backup: ${err?.message || err}`, 'error');
    }
  }
  ...
  function closeRestoreModal() {
    document.getElementById('nhRestoreBackupModal')?.remove();
  }

  function openRestorePreview(backup, fileName = 'backup.json') {
    validateBackupFile(backup);
    closeRestoreModal();
    const counts = backupTableCounts(backup);
    const total = Object.values(counts).reduce((s,n) => s + Number(n || 0), 0);
    const backdrop = document.createElement('div');
    backdrop.id = 'nhRestoreBackupModal';
    backdrop.className = 'nh-modal-backdrop';
    const rows = [
      ['Clientes / leads', counts.leads],
      ['Anúncios', counts.listings],
      ['Negociações / projetos', counts.deals],
      ['Pagamentos', counts.deal_payments],
      ['Parcelas', counts.deal_installments],
      ['Histórico de clientes', counts.client_history],
      ['Metas comerciais', counts.commercial_goals],
      ['Ações de cobrança', counts.deal_collection_log]
    ];
    backdrop.innerHTML = `
      <div class="nh-modal" role="dialog" aria-modal="true" aria-label="Restaurar backup">
        <div class="nh-modal-head">
          <div><h3>Restaurar backup</h3><div class="muted">${esc(fileName)}</div></div>
          <button type="button" class="nh-close" id="nhCloseRestoreBackup" aria-label="Fechar">×</button>
        </div>
        <div class="nh-project-note"><strong>Restauração segura por mesclagem</strong><br>Os dados do arquivo serão recuperados sem apagar registros mais novos que não estejam no backup.</div>
        <div class="nh-schedule-total" style="margin-top:12px">
          <div><small>Backup gerado em</small><strong>${esc(dateTimeBR(backup.exported_at))}</strong></div>
          <div><small>Registros no arquivo</small><strong>${esc(total)}</strong></div>
        </div>
        <div class="nh-history" style="margin-top:8px">
          ${rows.map(([label,n]) => `<div class="nh-history-row"><span>${esc(label)}</span><strong>${esc(n)}</strong></div>`).join('')}
        </div>
        <div class="nh-form-note" style="margin:12px 0">A operação exige conta administrativa e é executada em uma única transação no Supabase. Se houver erro, nenhuma restauração parcial é mantida.</div>
        <div id="nhRestoreBackupError"></div>
        <button type="button" class="nh-submit" id="nhConfirmRestoreBackup">RESTAURAR / MESCLAR DADOS</button>
      </div>`;
    document.body.appendChild(backdrop);
    document.getElementById('nhCloseRestoreBackup')?.addEventListener('click', closeRestoreModal);
    backdrop.addEventListener('click', e => { if (e.target === backdrop) closeRestoreModal(); });
    document.getElementById('nhConfirmRestoreBackup')?.addEventListener('click', async (e) => {
      const btn = e.currentTarget;
      const errorBox = document.getElementById('nhRestoreBackupError');
      try {
        btn.disabled = true;
        btn.textContent = 'RESTAURANDO…';
        if (errorBox) errorBox.innerHTML = '';
        const { data, error } = await db.rpc('restore_nomad_backup', { p_backup: backup });
        if (error) throw error;
        localStorage.setItem('nhSecureLastRestoreAt', new Date().toISOString());
        btn.textContent = 'RESTAURAÇÃO CONCLUÍDA';
        await loadFinance();
        setTimeout(() => {
          closeRestoreModal();
          alert('Backup restaurado com sucesso. Os dados foram mesclados com segurança.');
          window.location.reload();
        }, 450);
      } catch (err) {
        console.error('[Nomad Horse Backup] restauração', err);
        if (errorBox) errorBox.innerHTML = `<div class="nh-error">Não foi possível restaurar: ${esc(err?.message || err)}</div>`;
        btn.disabled = false;
        btn.textContent = 'RESTAURAR / MESCLAR DADOS';
      }
    });
  }

  function ensureRestoreInput() {
    let input = document.getElementById('nhRestoreBackupInput');
    if (input) return input;
    input = document.createElement('input');
    input.id = 'nhRestoreBackupInput';
    input.type = 'file';
    ...
    input.addEventListener('change', async () => {
      ...
      try {
        ...
        openRestorePreview(backup, file.name);
      } catch (err) {
        console.error('[Nomad Horse Backup] leitura', err);
        alert(`Arquivo de backup inválido. ${err?.message || err}`);
      }
    });
  ...
  function openRestorePicker() {
    if (!db) {
      alert('Aguarde o painel terminar de carregar.');
      return;
    }
    ensureRestoreInput().click();
  }

  function injectBackupControls() {
    const admin = getAdminSection();
    if (!admin) return;
    const allButtons = [...admin.querySelectorAll('button')];
    const exportBtn = allButtons.find(btn => /exportar backup/i.test(btn.textContent || ''));
    if (exportBtn && exportBtn.dataset.nhSecureBackup !== '1') {
      exportBtn.dataset.nhSecureBackup = '1';
      exportBtn.removeAttribute('onclick');
      exportBtn.onclick = null;
      exportBtn.addEventListener('click', exportSecureBackup);
    }
    if (exportBtn && !document.getElementById('nhRestoreBackupBtn')) {
      const restoreBtn = document.createElement('button');
      restoreBtn.id = 'nhRestoreBackupBtn';
      restoreBtn.type = 'button';
      restoreBtn.className = exportBtn.className || 'btn ghost';
      restoreBtn.textContent = 'Restaurar backup';
      restoreBtn.addEventListener('click', openRestorePicker);
      exportBtn.insertAdjacentElement('afterend', restoreBtn);
    }
    if (!exportBtn && !document.getElementById('nhRestoreBackupBtn')) {
      const head = document.querySelector('#nhFinancePanel .nh-project-head-actions');
      if (head) {
        const exportFallback = document.createElement('button');
        exportFallback.type = 'button';
        exportFallback.className = 'nh-mini-btn secondary';
        exportFallback.textContent = 'Exportar backup';
        exportFallback.addEventListener('click', exportSecureBackup);
        const restoreFallback = document.createElement('button');
        restoreFallback.id = 'nhRestoreBackupBtn';
        restoreFallback.type = 'button';
        restoreFallback.className = 'nh-mini-btn secondary';
        restoreFallback.textContent = 'Restaurar backup';
        restoreFallback.addEventListener('click', openRestorePicker);
        head.append(exportFallback, restoreFallback);
      }
    }
  ...
  async function init() {
    try {
      ...
      window.nomadHorseFinance = { reload: loadFinance, client: db, editSchedule: openScheduleModal, newProject: openProjectSetupModal, registerPayment: openPaymentModal, paymentHistory: openPaymentHistoryModal, receiptPdf: generateReceiptPdf, shareReceipt, collectionDashboard: buildCollectionDashboard, exportBackup: exportSecureBackup, restoreBackup: openRestorePicker };
      window.exportData = exportSecureBackup;
      ...
      document.addEventListener('click', (e) => {
        ...
        if (el) setTimeout(() => { loadFinance(); injectBackupControls(); }, 120);
      });
      const projectObserver = new MutationObserver(() => { scheduleLeadProjectAction(); injectBackupControls(); });
