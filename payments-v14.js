...
(() => {
        ...
        ? `<div class="nh-schedule">
            ...
            ${deal.schedule.map(item => {
              ...
              const chargeButton = canCharge
                ? `<button type="button" class="nh-charge-btn ${status === 'atrasado' ? 'overdue' : ''}" data-charge-whatsapp="${esc(deal.id)}" data-installment="${esc(item.installment_number)}">Cobrar no WhatsApp</button>`
                : '';
              return `<div class="nh-schedule-row">
                <div class="nh-schedule-main"><strong>${esc(label)} #${esc(item.installment_number)}</strong><small>Vencimento: ${esc(due)}${paidInfo}</small>${chargeButton}</div>
  ...
  function openChargeWhatsApp(dealId, installmentNumber) {
    ...
    backdrop.innerHTML = `
      <div class="nh-modal" role="dialog" aria-modal="true" aria-label="Escolher WhatsApp">
        ...
        <div class="nh-form-note">Escolha qual aplicativo deve enviar esta cobrança. Nenhum pagamento será registrado ao abrir o WhatsApp.</div>
    ...
    document.getElementById('nhChargePersonal')?.addEventListener('click', () => {
      openWhatsAppPackage('com.whatsapp', phone, text);
    });
    document.getElementById('nhChargeBusiness')?.addEventListener('click', () => {
      openWhatsAppPackage('com.whatsapp.w4b', phone, text);
    });
    document.getElementById('nhChargeCopy')?.addEventListener('click', (e) => copyChargeMessage(text, e.currentTarget));
  }
  ...
  async function init() {
    try {
      ...
      window.nomadHorseFinance = { reload: loadFinance, client: db, editSchedule: openScheduleModal, newProject: openProjectSetupModal, registerPayment: openPaymentModal, paymentHistory: openPaymentHistoryModal, receiptPdf: generateReceiptPdf, shareReceipt };
      await loadFinance();
