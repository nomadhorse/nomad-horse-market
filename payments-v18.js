...
(() => {
  ...
  async function exportSecureBackup() {
    ...
    try {
      ...
      downloadJson(`nomad-horse-backup-${backupFileStamp()}.json`, data);
      ...
      alert(`Backup exportado com sucesso. ${total} registro(s) incluído(s).`);
    } catch (err) {
      console.error('[Nomad Horse Backup] exportação', err);
      alert(`Não foi possível exportar o backup. ${err?.message || err}`);
    }
  }
