/* Nomad Horse Market — financeiro + ciclo de vida de projetos (v22)
...
(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .nh-badge-stack{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}
      .nh-lifecycle-badge{display:inline-flex;align-items:center;border-radius:999px;padding:5px 9px;font-size:.72rem;font-weight:900;text-transform:uppercase;letter-spacing:.03em;background:rgba(200,156,74,.12);border:1px solid rgba(200,156,74,.26);color:#f0c66a}
      .nh-lifecycle-badge.concluido{background:rgba(45,150,85,.13);border-color:rgba(70,190,110,.28);color:#bde9c9}
      .nh-archive-section{margin-top:16px}
      .nh-archive-details{border:1px solid rgba(255,255,255,.09);border-radius:16px;background:rgba(255,255,255,.025);overflow:hidden}
      .nh-archive-details>summary{cursor:pointer;list-style:none;padding:14px 15px;font-weight:900;display:flex;align-items:center;justify-content:space-between;gap:12px}
      .nh-archive-details>summary::-webkit-details-marker{display:none}
      .nh-archive-count{display:inline-flex;align-items:center;justify-content:center;min-width:28px;height:28px;padding:0 8px;border-radius:999px;background:rgba(255,255,255,.08);font-size:.8rem}
      .nh-archive-list{display:grid;gap:10px;padding:0 12px 12px}
      .nh-archive-card{padding:13px;border-radius:13px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08)}
      .nh-archive-card-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}
      .nh-archive-card small{display:block;opacity:.66;margin-top:3px}
      .nh-archive-money{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:10px 0}
      .nh-archive-money>div{padding:9px 10px;border-radius:10px;background:rgba(255,255,255,.045)}
      .nh-archive-money small{margin:0 0 3px}
      .nh-lifecycle-summary{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:12px 0}
      .nh-lifecycle-summary>div{padding:10px;border-radius:11px;background:rgba(255,255,255,.05)}
      .nh-lifecycle-summary small{display:block;opacity:.65;margin-bottom:4px}
      .nh-archive-check{display:flex;gap:10px;align-items:flex-start;padding:11px 12px;border-radius:11px;background:rgba(200,156,74,.07);border:1px solid rgba(200,156,74,.20)}
      .nh-archive-check input{width:auto;margin-top:3px}
      .nh-archive-check.disabled{opacity:.55}
      @media(max-width:620px){.nh-collection-stats{grid-template-columns:1fr 1fr}.nh-collection-row{grid-template-columns:1fr}.nh-collection-action{justify-self:start}.nh-money-row{grid-template-columns:1fr}.nh-finance-summary{grid-template-columns:1fr}.nh-overview{grid-template-columns:1fr 1fr}.nh-form-grid{grid-template-columns:1fr}.nh-form-grid .full{grid-column:auto}.nh-lifecycle-summary{grid-template-columns:1fr}.nh-archive-money{grid-template-columns:1fr}.nh-schedule-row{grid-template-columns:1fr auto}.nh-schedule-money{grid-column:1/-1;text-align:left}}
    `;
  ...
  function injectPanel() {
    ...
    panel.innerHTML = `
      ...
      <div id="nhArchivedProjects" class="nh-archive-section"></div>
    `;
  ...
  async function loadFinance() {
    ...
    try {
      ...
      const { data: deals, error: dealsError } = await db
        .from('deals')
        .select('id,buyer_lead_id,seller_lead_id,stage,project_value,agreed_price,installment_count,payment_status,notes,created_at,project_status,delivered_at,completed_at,archived_at,archive_note')
        .is('archived_at', null)
      ...
      if (!deals?.length) {
        dealsCache = [];
        body.innerHTML = '<div class="nh-empty">Nenhum projeto financeiro ativo.</div>';
  ...
  function renderFinance() {
    ...
    const html = dealsCache.map(deal => {
      ...
      const lifecycleStatus = String(deal.project_status || 'ativo').toLowerCase();
      const lifecycleLabel = lifecycleStatus === 'entregue' ? 'Entregue' : lifecycleStatus === 'concluido' ? 'Concluído' : '';
      const lifecycleBadge = lifecycleLabel ? `<span class="nh-lifecycle-badge ${esc(lifecycleStatus)}">${esc(lifecycleLabel)}</span>` : '';
      ...
      return `
        <article class="nh-deal-card" data-deal-id="${esc(deal.id)}">
          <div class="nh-deal-top">
            ...
            <div class="nh-badge-stack"><span class="nh-badge">${esc(displayStatus)}</span>${lifecycleBadge}</div>
          </div>
          ...
          <div class="nh-deal-actions" style="margin-top:14px">
            ...
            <button type="button" class="nh-mini-btn secondary" data-project-lifecycle="${esc(deal.id)}">Concluir / arquivar</button>
          </div>
    ...
    body.querySelectorAll('[data-project-lifecycle]').forEach(btn => {
      btn.addEventListener('click', () => openProjectLifecycleModal(btn.dataset.projectLifecycle));
    });
  ...
  function projectStatusLabel(value) {
    const v = String(value || 'ativo').toLowerCase();
  ...
  function closeProjectLifecycleModal() {
    document.getElementById('nhProjectLifecycleModal')?.remove();
  }

  function openProjectLifecycleModal(dealId) {
    const deal = dealsCache.find(d => String(d.id) === String(dealId));
    if (!deal) return;
    closeProjectLifecycleModal();
    const canArchive = Number(deal.balance || 0) <= 0.009 && Number(deal.total || 0) > 0;
    const currentStatus = String(deal.project_status || 'ativo').toLowerCase();
    const wrap = document.createElement('div');
    wrap.id = 'nhProjectLifecycleModal';
    wrap.className = 'nh-modal-backdrop';
    wrap.innerHTML = `
      <div class="nh-modal" role="dialog" aria-modal="true" aria-label="Situação do projeto">
        <div class="nh-modal-head">
          <div><h3>Concluir / arquivar projeto</h3><div class="muted">${esc(deal.customer?.name || 'Cliente')}</div></div>
          <button type="button" class="nh-close" id="nhCloseLifecycle">×</button>
        </div>
        <div class="nh-lifecycle-summary">
          <div><small>Projeto</small><strong>${money(deal.total)}</strong></div>
          <div><small>Recebido</small><strong>${money(deal.paid)}</strong></div>
          <div><small>Saldo</small><strong>${money(deal.balance)}</strong></div>
        </div>
        <form id="nhLifecycleForm" class="nh-form-grid">
          <div class="full">
            <label>Situação do projeto</label>
            <select name="project_status">
              <option value="ativo" ${currentStatus === 'ativo' ? 'selected' : ''}>Em andamento</option>
          ...
          <div class="full">
            ...
            <textarea name="note" placeholder="Ex.: veículo entregue ao cliente, projeto encerrado…">${esc(deal.archive_note || '')}</textarea>
          </div>
          <label class="full nh-archive-check ${canArchive ? '' : 'disabled'}">
            <input type="checkbox" name="archive" value="1" ${canArchive ? '' : 'disabled'}>
            <span><strong>Arquivar após salvar</strong><br><small>${canArchive ? 'O projeto sairá da lista ativa, mas todo o histórico ficará preservado.' : `Arquivamento disponível somente após a quitação. Saldo atual: ${money(deal.balance)}.`}</small></span>
          </label>
          <div class="full nh-form-note">“Quitado” é definido automaticamente pelos pagamentos registrados. Você não precisa marcar manualmente.</div>
          <div id="nhLifecycleError" class="full"></div>
          <div class="full"><button type="submit" class="nh-submit">SALVAR SITUAÇÃO</button></div>
        </form>
      </div>`;
    document.body.appendChild(wrap);
    document.getElementById('nhCloseLifecycle')?.addEventListener('click', closeProjectLifecycleModal, { once: true });
    document.getElementById('nhLifecycleForm')?.addEventListener('submit', (e) => saveProjectLifecycle(e, deal));
  }

  async function saveProjectLifecycle(event, deal) {
    event.preventDefault();
    const form = event.currentTarget;
    const btn = form.querySelector('button[type="submit"]');
    const errorBox = document.getElementById('nhLifecycleError');
    const fd = new FormData(form);
    const status = String(fd.get('project_status') || 'ativo');
    ...
    try {
      ...
      const { error } = await db.rpc('update_deal_project_lifecycle', {
        p_deal_id: deal.id,
        p_status: status,
        p_archive: archive,
        p_note: note
      ...
      console.error('[Nomad Horse Finance] ciclo de vida', err);
      if (errorBox) errorBox.innerHTML = `<div class="nh-error">Não foi possível salvar: ${esc(err?.message || err)}</div>`;
      btn.disabled = false;
      btn.textContent = 'SALVAR SITUAÇÃO';
    }
  }

  async function reopenArchivedProject(dealId, button) {
    try {
      if (button) { button.disabled = true; button.textContent = 'REABRINDO…'; }
      const { error } = await db.rpc('update_deal_project_lifecycle', {
        p_deal_id: dealId,
        p_status: 'ativo',
        p_archive: false,
        p_note: 'Projeto reaberto pelo painel.'
      });
      if (error) throw error;
      await loadFinance();
    } catch (err) {
      console.error('[Nomad Horse Finance] reabrir projeto', err);
      alert(`Não foi possível reabrir o projeto. ${err?.message || ''}`);
      if (button) { button.disabled = false; button.textContent = 'Reabrir projeto'; }
    }
  }

  async function loadArchivedProjects() {
    const host = document.getElementById('nhArchivedProjects');
    if (!host || !db) return;
    try {
      const { data: deals, error } = await db
        .from('deals')
        .select('id,buyer_lead_id,seller_lead_id,project_value,agreed_price,project_status,archived_at,archive_note,created_at')
        .not('archived_at', 'is', null)
        .order('archived_at', { ascending: false });
      if (error) throw error;
      if (!deals?.length) {
        archivedDealsCache = [];
        host.innerHTML = `<details class="nh-archive-details"><summary><span>Projetos arquivados</span><span class="nh-archive-count">0</span></summary><div class="nh-archive-list"><div class="nh-empty">Nenhum projeto arquivado.</div></div></details>`;
        return;
      }
      const leadIds = [...new Set(deals.flatMap(d => [d.buyer_lead_id,d.seller_lead_id]).filter(Boolean))];
      const dealIds = deals.map(d => d.id);
      const [leadRes,payRes] = await Promise.all([
        leadIds.length ? db.from('leads').select('id,name,phone').in('id',leadIds) : Promise.resolve({data:[],error:null}),
        db.from('deal_payments').select('deal_id,amount').in('deal_id',dealIds)
      ]);
      if (leadRes.error) throw leadRes.error;
      if (payRes.error) throw payRes.error;
      const leads = new Map((leadRes.data || []).map(l => [l.id,l]));
      const paidByDeal = new Map();
      (payRes.data || []).forEach(p => paidByDeal.set(p.deal_id, Number(paidByDeal.get(p.deal_id) || 0) + Number(p.amount || 0)));
      archivedDealsCache = deals.map(d => {
        const total = Number(d.project_value ?? d.agreed_price ?? 0);
        const paid = Number(paidByDeal.get(d.id) || 0);
        return {...d, customer:leads.get(d.buyer_lead_id)||leads.get(d.seller_lead_id)||{}, total, paid, balance:Math.max(total-paid,0)};
      });
      host.innerHTML = `<details class="nh-archive-details"><summary><span>Projetos arquivados</span><span class="nh-archive-count">${archivedDealsCache.length}</span></summary><div class="nh-archive-list">${archivedDealsCache.map(deal => `
        <div class="nh-archive-card">
          <div class="nh-archive-card-head"><div><strong>${esc(deal.customer?.name || 'Cliente')}</strong><small>Arquivado em ${esc(dateTimeBR(deal.archived_at))}</small></div><span class="nh-lifecycle-badge concluido">${esc(projectStatusLabel(deal.project_status))}</span></div>
          <div class="nh-archive-money"><div><small>Projeto</small><strong>${money(deal.total)}</strong></div><div><small>Recebido</small><strong>${money(deal.paid)}</strong></div></div>
          ${deal.archive_note ? `<small>${esc(deal.archive_note)}</small>` : ''}
          <div class="nh-deal-actions" style="margin-top:10px"><button type="button" class="nh-mini-btn secondary" data-reopen-project="${esc(deal.id)}">Reabrir projeto</button></div>
        </div>`).join('')}</div></details>`;
      host.querySelectorAll('[data-reopen-project]').forEach(btn => btn.addEventListener('click', () => reopenArchivedProject(btn.dataset.reopenProject, btn)));
    } catch (err) {
      console.error('[Nomad Horse Finance] projetos arquivados', err);
      host.innerHTML = `<div class="nh-error">Não foi possível carregar os projetos arquivados: ${esc(err?.message || err)}</div>`;
    }
  }
  ...
  async function projectContext() {
    const [leadRes, dealRes] = await Promise.all([
      ...
      db.from('deals').select('id,buyer_lead_id,seller_lead_id,project_value,agreed_price,created_at,archived_at').is('archived_at',null).order('created_at',{ascending:false})
    ]);
    if (leadRes.error) throw leadRes.error;
  ...
  async function injectLeadProjectAction() {
    ...
    try {
      const { data: deals, error } = await db.from('deals').select('id,project_value,agreed_price,archived_at').or(`buyer_lead_id.eq.${leadId},seller_lead_id.eq.${leadId}`).is('archived_at',null).order('created_at',{ascending:false}).limit(1);
      if (error) return;
  ...
  async function init() {
    try {
      ...
      window.nomadHorseFinance = { reload: loadFinance, client: db, editSchedule: openScheduleModal, newProject: openProjectSetupModal, registerPayment: openPaymentModal, paymentHistory: openPaymentHistoryModal, receiptPdf: generateReceiptPdf, shareReceipt, collectionDashboard: buildCollectionDashboard, archived: loadArchivedProjects, projectLifecycle: openProjectLifecycleModal };
      await loadFinance();
