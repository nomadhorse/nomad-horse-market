...
(() => {
  ...
  function openScheduleModal(dealId) {
    ...
    const rows = deal.schedule.map(item => {
      ...
      return `
        <div class="nh-schedule-edit-row" data-schedule-row data-id="${esc(item.id)}" data-number="${esc(item.installment_number)}" data-paid="${esc(paid)}">
          <div class="nh-schedule-edit-head">
            <strong>${esc(labelPaymentType(item.kind))} #${esc(item.installment_number)}</strong>
            ${paid > 0 ? `<span class="nh-badge">Pago ${money(paid)}</span>` : ''}
          </div>
          <div class="nh-schedule-edit-grid">
            <div>
              ...
              <select name="kind">
                ...
                <option value="parcela" ${item.kind === 'parcela' ? 'selected' : ''}>Parcela</option>
    ...
    backdrop.innerHTML = `
      <div class="nh-modal" role="dialog" aria-modal="true" aria-label="Editar cronograma">
        ...
        <form id="nhScheduleForm">
          <div class="nh-form-note">Altere valores e vencimentos. Para proteger o contrato, a soma das parcelas precisa continuar igual ao total do projeto.</div>
          <div class="nh-schedule-editor">${rows}</div>
    ...
    form?.querySelectorAll('input[name="amount"]').forEach(input => {
      input.addEventListener('input', () => updateScheduleTotal(deal));
    });
    form?.querySelectorAll('input[name="due_date"]').forEach(input => {
      input.addEventListener('change', () => {
        ...
        if (input.value && label) label.value = '';
      });
    });
    form?.querySelectorAll('input[name="due_label"]').forEach(input => {
      input.addEventListener('input', () => {
        const row = input.closest('[data-schedule-row]');
        const date = row?.querySelector('input[name="due_date"]');
        if (input.value.trim() && date) date.value = '';
      });
    });
  ...
  function updateScheduleTotal(deal) {
    ...
    if (!totalEl) return;
    const total = scheduleRowsFromForm().reduce((sum, row) => sum + Number(row.amount || 0), 0);
  ...
  async function saveSchedule(event, deal) {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const { error } = await db.from('deal_installments').update(payload).eq('id', row.id);
